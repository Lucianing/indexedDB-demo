(function () {
  ...
  var eleForm = $('#form');
  var eleTbody = $('#result tbody');
  var eleStatus = $('#status');
  var searchBtn = $('#search');
  var eleSearchInput = $('#search-input');
  ...
  var method = {
    ...
    show: function (name) {
      ...
      objectStore.openCursor().onsuccess = function (event) {
        ...
        if (cursor) {
          if (name) {
            if (cursor.value.name.indexOf(name) !== -1) {
  ...
  eleTbody.addEventListener('focusout', function (event) {
    ...
    if (!id || eleTd.tagName.toLowerCase() !== 'td') {
      return;
    }
  ...
  searchBtn.addEventListener('click', function(event) {
    var searchStr = eleSearchInput.value;
    if (searchStr) {
      method.show(searchStr);
    }
  })
